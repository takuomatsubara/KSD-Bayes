...
parser.add_argument('--pv', default=5.0, type=float)
parser.add_argument('--pnum', default=1000, type=int)
args = parser.parse_args()

File_ID = 'KSD_theta=' + str(args.theta) + '_data=' + str(args.data) + '_beta=' + str(args.beta) + '_noise=' + str(args.noise) + '_pv=' + str(args.pv) + '_pnum=' + str(args.pnum)
...
class KSD_Bayes():
    ...
    def __init__(self, score, log_prior, beta=1.0, gamma=1.0):
    ...
    def HK(self, X):
        return torch.exp( - torch.cdist(X, X, p=0) / X.shape[1] * self.gamma )
    ...
    def HK_D(self, X):
        ...
        X1_X2_m = torch.cdist(X, X_m, p=0) / X.shape[1] * self.gamma
        return ( self.HK_XX.unsqueeze(-1) - torch.exp(-X1_X2_m) ).mean(axis=1)
...
ksd_bayes = KSD_Bayes(model_ising.score, prior.log_prob, beta=args.beta)
ksd_bayes.set_KX(X)
